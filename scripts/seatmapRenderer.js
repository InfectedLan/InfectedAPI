...
function renderSeatmap(target, seatHandlerFunction, callback) {
	...
	for(var i = 0; i < seatmapData.rows.length; i++)
	{
		...
		for(var s = 0; s < seatmapData.rows[i].seats.length; s++)
		{
			...
			var customClass = (seatmapData.rows[i].seats[s].occupied && seatmapData.rows[i].seats[s].occupiedTicket.isFriend)?"friend":(seatmapData.rows[i].seats[s].occupied ? "taken" : "free");
