			...
			if ($note->hasOwner() || $note->hasUser()) {
				...
				$message[] = '<html>';
					$message[] = '<body>';
						...
						$message[] = '<p>Med vennlig hilsen <a href="https://' . Settings::getValue("domain") . '/">Infected</a>.</p>';
					$message[] = '</body>';
				$message[] = '</html>';
		...
		if (!empty($userList)) {
			...
			$message[] = '<html>';
				$message[] = '<body>';
					...
					$message[] = '<p>Klikk <a href="https://crew.' . Settings::getValue("domain") . '/v2/index.php?page=application&id=' . $application->getId() . '">her</a> for å se den.</p>';
					$message[] = '<p>Med vennlig hilsen <a href="https://' . Settings::getValue("domain") . '/">Infected</a>.</p>';
				$message[] = '</body>';
			$message[] = '</html>';
	...
	public function sendApplicationAccpetedNotification(Application $application) {
		...
		$message[] = '<html>';
			$message[] = '<body>';
				...
				$message[] = 'Du kan nå logge inn på <a href="https://crew.' . Settings::getValue("domain") . '/">Infected Crew</a> å bli kjent med det nye crewet ditt.<br>';
	...
	public function sendPurchaseCompleteNotification(User $user, string $reference) {
		...
		$message[] = '<html>';
			$message[] = '<body>';
				...
				$message[] = '<p>Du kan nå plassere deg ved å trykke <a href="https://tickets.' . Settings::getValue("domain") . '/v2/index.php?page=viewSeatmap">her</a>.</p>';
				$message[] = '<p>Med vennlig hilsen <a href="https://' . Settings::getValue("domain") . '/">Infected</a>.</p>';
			$message[] = '</body>';
		$message[] = '</html>';
