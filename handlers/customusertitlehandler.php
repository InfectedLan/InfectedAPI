...
class CustomUserTitleHandler {
    ...
    public static function hasCustomTitle(User $user): bool {
        ...
        $result = $database->query('SELECT * FROM `' . Settings::db_table_infected_crew_customusertitles . '`
    ...
    public static function getCustomTitle(User $user): string {
        ...
        $result = $database->query('SELECT `title` FROM `' . Settings::db_table_infected_crew_customusertitles . '`
																WHERE `userId` = \'' . $user->getId() . '\';');
    ...
    public static function setCustomTitle(User $user, string $title) {
        ...
        if(self::hasCustomTitle($user)) {
            ...
            $result = $database->query('INSERT INTO `' . Settings::db_table_infected_crew_customusertitles . '` (`userId`, `title`) VALUES \'' . $user->getId() . '\', \'' . $database->real_escape_string($title) . '\';');
        }
    }
}
?>
